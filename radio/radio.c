...
uint_fast8_t PHY_open(QueueHandle_t handle){
    g_rx_queue = handle;
    //***************************************************************************
    //***************************************************************************
    //                                Start the radio
    ...
    bool interruptsDisabled = IntMasterDisable();
    ...
    IntPrioritySet(INT_RFC_CPE_0, INT_PRI_LEVEL1); // This is needed because FreeRTOS will not work if there is a priority higher than System (0)
    ...
    RF_cmdPropRx.pktConf.bVarLen            = 0x01; // Receive length as first byte
    // Run RX continuously
    ...
    RF_cmdPropTx.pktConf.bVarLen            = 0x01; // Transmit length as first byte
...
void RFCCPE0IntHandler(){
    ...
    if (HWREG(RFC_DBELL_NONBUF_BASE + RFC_DBELL_O_RFCPEIFG) & IRQ_RX_ENTRY_DONE){
        ...
        // Send the address of the data entry to the task that is watching the queue
        BaseType_t xHigherPriorityTaskAwakend = pdFALSE;
        xQueueSendFromISR(g_rx_queue, &g_rx_entry, &xHigherPriorityTaskAwakend);

        // Move to next entry
        g_rx_entry = entry->pNextEntry;
        HWREG(RFC_DBELL_NONBUF_BASE + RFC_DBELL_O_RFCPEIFG) = ~IRQ_RX_ENTRY_DONE;
        portYIELD_FROM_ISR(xHigherPriorityTaskAwakend);
    }
}
